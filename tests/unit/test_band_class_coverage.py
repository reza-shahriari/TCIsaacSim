...
5.5× change** in how much of the band is treated as a water wing. LWIR barely moves, because the
Boson's response happens to be close to its nominal top-hat — which is exactly why this stayed
invisible: the one band the project renders most is the one the shortcut suits.
...
from irsim.atmosphere.layered import BAND_CLASSES, LayeredAtmosphere, class_weights
...
def test_every_shipped_sensor_response_is_inside_its_bands_classes(path: pathlib.Path) -> None:
    ...
    assert weights.shape == (len(BAND_CLASSES[band]),)
...
def test_the_class_edges_span_the_detector(band: str) -> None:
    ...
    SWIR (short both sides) and MWIR (short by 0.4 µm) carried 0.000 % and passed on luck.
    """
    response = load_spectral_response(RESPONSES / BAND_FILES[band])
    edges = [e for c in BAND_CLASSES[band] for e in c.edges_um]
...
def test_the_nir_tail_reaches_where_swir_begins() -> None:
    """The new NIR edge is not arbitrary: it meets SWIR's own first class at 1.10 µm."""
    nir_high = max(e[1] for c in BAND_CLASSES["nir"] for e in c.edges_um)
    swir_low = min(e[0] for c in BAND_CLASSES["swir"] for e in c.edges_um)
    assert nir_high == 1.10
    assert swir_low <= nir_high


# --- AT.2: the response changes the answer, and the fallback is loud ---------------------------


def test_the_mwir_water_wing_weight_moves_by_five_and_a_half_times() -> None:
    """The measured defect, recorded as a number so a later change cannot quietly undo it."""
    names = [c.name for c in BAND_CLASSES["mwir"]]
    index = names.index("h2o_wing")
    nominal = class_weights("mwir")[index]
    real = class_weights("mwir", load_spectral_response(RESPONSES / "insb.csv"))[index]
    assert nominal == pytest.approx(0.0238, abs=5e-4)
    assert real == pytest.approx(0.1303, abs=5e-4)
    assert real / nominal > 5.0
...
def test_a_layered_atmosphere_with_a_response_is_quiet_and_different() -> None:
    ...
    with warnings.catch_warnings():
        ...
        supplied = LayeredAtmosphere(preset, weather, None, {"mwir": response}).weights("mwir")
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        fallback = LayeredAtmosphere(preset, weather).weights("mwir")
    assert not np.allclose(supplied, fallback)
