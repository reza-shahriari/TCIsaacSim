...
| `atmosphere` | 🟡 partial | T1 | Beer–Lambert kernel (isothermal invariance to 1e-12), Magnus humidity, grey-band error study (ADR 0048), seven presets reproducing the §7.2 table rows with Koschmieder aerosol (ADR 0049: fog ordering and the humid/fog LWIR–SWIR crossover from weather alone); WeatherSeries binding (M8.5) and the pipeline stage (M8.6) pending |
