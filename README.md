...
**First light in Isaac Sim.** `IrCamera` (M10.9a-ii) renders the five-prim material stage through
the whole camera model; `tests/integration/test_ir_camera_isaac.py` is the closure. The lens is
verified against the renderer to 0.2 px, and the frame's four outputs come back at the declared
dtypes. What is still missing for a *scene* rather than a test stage is a committed sky-dome demo
asset (M10.19) and the float32 dataset writers (M10.10a).
