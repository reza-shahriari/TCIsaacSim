...
| `isp` | 🟡 partial | T1 | Radiometric branch (ADR 0021); linear AGC (ADR 0027), plateau equalisation with the hot-exhaust collapse as a scalar test (ADR 0028), DDE (ADR 0029), polarity + palettes → RGBA8 (ADR 0030); display branch assembled in the fixed order with documented rounding points and the isp config hash (ADR 0031); ideal two-point NUC operator; stage 6 wiring pending |
