...
### Start here → `IG.6` · Isaac glue integrity

> **Wire `MotionTracker` into `IrCamera`.** M10.1b built it and verified it in-sim to 0.1 px, and its only caller is the test — so `capture()` never sets `aovs.motion` and ADR 0077's smear has never run on a rendered frame, with M9.8 and M10.1b both ticked.

`IG.6` is phase A, size S, and unblocks 0 other step(s).
...
| 5 | **`IG.2`** | IG | A | M | — | ready |
| 6 | **`SC.4`** | SC | A | M | — | ready |
| 7 | **`WM.1`** | WM | B | M | 10 | ready |
| 8 | **`WM.2`** | WM | B | M | 9 | `WM.1` |
| 9 | **`WM.3`** | WM | B | M | 8 | `WM.2` |
| 10 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 11 | **`WM.5`** | WM | B | S | 1 | `WM.3` |
