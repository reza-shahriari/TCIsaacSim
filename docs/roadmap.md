...
### Start here → `PT.3` · Point-wise surface temperature

> **Fix the nested-rectangle double count.** `build_ground_field` sums `occluded_longwave_flux` over underbody, engine-bay floor and exhaust, which are geometrically nested. Partition them, or clamp with a loud warning.

`PT.3` is phase 0, size S, and unblocks 6 other step(s).

#### Then, in order — 108 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`PT.3`** | PT | 0 | S | 6 | ready |
| 2 | **`IG.1`** | IG | 0 | S | 2 | ready |
| 3 | **`RP.4`** | RP | 0 | M | 2 | ready |
| 4 | **`RP.7`** | RP | 0 | M | 2 | ready |
| 5 | **`IG.5`** | IG | 0 | S | 1 | ready |
| 6 | **`RP.6`** | RP | 0 | M | 1 | ready |
| 7 | **`IG.8`** | IG | 0 | S | — | ready |
| 8 | **`PT.4`** | PT | 0 | S | — | ready |
| 9 | **`RP.8`** | RP | 0 | S | — | ready |
| 10 | **`RP.9`** | RP | 0 | S | — | ready |
| 11 | **`RP.2`** | RP | 0 | M | — | ready |
| 12 | **`RP.5`** | RP | 0 | M | — | ready |
| 13 | **`PT.2`** | PT | A | M | 7 | ready |
| 14 | **`PT.1`** | PT | A | M | 4 | ready |
| 15 | **`PT.5`** | PT | A | M | 4 | `PT.2` |

…and 93 more — `python scripts/next_step.py --queue 40`.
