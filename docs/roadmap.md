...
### Start here → `RP.3` · Repair

> Make `scripts/stage_own_hunk.sh` the default path, not an available one: `.pre-commit-config.yaml`, a `make stage` target, and the instruction in the `ship-step` skill. It was written and tested (8 cases) for exactly this failure and nothing reaches it.

`RP.3` is phase 0, size S, and unblocks 0 other step(s).

**Promoted over the mechanical order.** Makes stage_own_hunk.sh the default commit path. It unblocks nothing, so the mechanical key sorts it ninth -- but three sessions share this tree and a whole-file write has silently reverted committed work three times in two days (see the two `docs: restore ...` commits). Every session pays for it until it is fixed, and the cost is other people's shipped work.

#### Then, in order — 109 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`RP.3`** | RP | 0 | S | — | ready |
| 2 | **`PT.3`** | PT | 0 | S | 6 | ready |
| 3 | **`IG.1`** | IG | 0 | S | 2 | ready |
| 4 | **`RP.4`** | RP | 0 | M | 2 | ready |
| 5 | **`RP.7`** | RP | 0 | M | 2 | ready |
| 6 | **`IG.5`** | IG | 0 | S | 1 | ready |
| 7 | **`RP.6`** | RP | 0 | M | 1 | ready |
| 8 | **`IG.8`** | IG | 0 | S | — | ready |
| 9 | **`PT.4`** | PT | 0 | S | — | ready |
| 10 | **`RP.8`** | RP | 0 | S | — | ready |
| 11 | **`RP.9`** | RP | 0 | S | — | ready |
| 12 | **`RP.2`** | RP | 0 | M | — | ready |
| 13 | **`RP.5`** | RP | 0 | M | — | ready |
| 14 | **`PT.2`** | PT | A | M | 7 | ready |
| 15 | **`PT.1`** | PT | A | M | 4 | ready |

…and 94 more — `python scripts/next_step.py --queue 40`.
