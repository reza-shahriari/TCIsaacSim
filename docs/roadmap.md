...
### Start here → `RP.4` · Repair

> Write the two ADRs shipped code cites and does not have: the Level B angular-emissivity model (cited by `directional.py:21`, `angular.py:24` and four tests) and the sea-water n/k plus Cox–Munk slope model (cited by `sea.py:36`, `nk.py:23`, ADR 0078). Numbers allocated at write time.

`RP.4` is phase 0, size M, and unblocks 2 other step(s).

#### Then, in order — 106 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`RP.4`** | RP | 0 | M | 2 | ready |
| 2 | **`RP.7`** | RP | 0 | M | 2 | ready |
| 3 | **`IG.5`** | IG | 0 | S | 1 | ready |
| 4 | **`RP.6`** | RP | 0 | M | 1 | ready |
| 5 | **`IG.8`** | IG | 0 | S | — | ready |
| 6 | **`PT.4`** | PT | 0 | S | — | ready |
| 7 | **`RP.8`** | RP | 0 | S | — | ready |
| 8 | **`RP.9`** | RP | 0 | S | — | ready |
| 9 | **`RP.2`** | RP | 0 | M | — | ready |
| 10 | **`RP.5`** | RP | 0 | M | — | ready |
| 11 | **`PT.2`** | PT | A | M | 7 | ready |
| 12 | **`PT.1`** | PT | A | M | 4 | ready |
| 13 | **`PT.5`** | PT | A | M | 4 | `PT.2` |
| 14 | **`AT.1`** | AT | A | L | 4 | ready |
| 15 | **`GT.1`** | GT | A | M | 2 | ready |
