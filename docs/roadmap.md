...
### Start here → `PT.1` · Point-wise surface temperature

> **Per-cell solar and shadow.** ADR 0087's own headline case — a wall half in sun spanning 10–20 K — is unimplemented: nothing varies `q_solar_w_m2` or shadow across a surface, which is why both point-wise scenes shipped are pre-dawn. The per-facet direct/diffuse split exists; nothing joins it to a p

`PT.1` is phase A, size M, and unblocks 4 other step(s).

#### Then, in order — 95 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`PT.1`** | PT | A | M | 4 | ready |
| 2 | **`PT.5`** | PT | A | M | 4 | ready |
| 3 | **`AT.1`** | AT | A | L | 4 | ready |
| 4 | **`GT.1`** | GT | A | M | 2 | ready |
| 5 | **`IG.13`** | IG | A | M | 2 | ready |
| 6 | **`SC.1`** | SC | A | M | 2 | ready |
| 7 | **`AT.2`** | AT | A | S | 1 | ready |
| 8 | **`SC.3`** | SC | A | M | 1 | ready |
| 9 | **`AT.3`** | AT | A | S | — | `AT.2` |
| 10 | **`AT.5`** | AT | A | S | — | ready |
| 11 | **`IG.6`** | IG | A | S | — | ready |
| 12 | **`SC.2`** | SC | A | S | — | `SC.1` |
| 13 | **`AT.4`** | AT | A | M | — | ready |
| 14 | **`GT.2`** | GT | A | M | — | `SC.1` |
