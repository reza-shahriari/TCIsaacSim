...
### Start here → `AT.5` · Atmosphere, sky and materials

> **Guard the grey `Atmosphere`.** `Scene.from_config` builds both models unconditionally; `scene.atmosphere` is the primary attribute while every render script passes `scene.layered`, and `pipeline/atmosphere.py:87` branches between them. A caller taking the primary attribute gets a materially differ

`AT.5` is phase A, size S, and unblocks 0 other step(s).
...
| 6 | **`IG.2`** | IG | A | M | — | ready |
| 7 | **`SC.4`** | SC | A | M | — | ready |
| 8 | **`WM.1`** | WM | B | M | 10 | ready |
| 9 | **`WM.2`** | WM | B | M | 9 | `WM.1` |
| 10 | **`WM.3`** | WM | B | M | 8 | `WM.2` |
| 11 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 12 | **`WM.5`** | WM | B | S | 1 | `WM.3` |
