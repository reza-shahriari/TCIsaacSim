...
### Start here → `IG.5` · Isaac glue integrity

> **The bogus `motion_px` plane.** `gbuffer_isaac.py` documents `motion_px` as omitted; it is not. `motion_vectors` sits at a ~6e-5 floor and `_reject_reason` applies its all-zero check only to *required* channels, so the plane reaches the G-buffer and the optics stage runs the smear path on it.

`IG.5` is phase 0, size S, and unblocks 1 other step(s).

#### Then, in order — 104 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`IG.5`** | IG | 0 | S | 1 | ready |
| 2 | **`RP.6`** | RP | 0 | M | 1 | ready |
| 3 | **`IG.8`** | IG | 0 | S | — | ready |
| 4 | **`PT.4`** | PT | 0 | S | — | ready |
| 5 | **`RP.8`** | RP | 0 | S | — | ready |
| 6 | **`RP.9`** | RP | 0 | S | — | ready |
| 7 | **`RP.2`** | RP | 0 | M | — | ready |
| 8 | **`RP.5`** | RP | 0 | M | — | ready |
| 9 | **`PT.2`** | PT | A | M | 7 | ready |
| 10 | **`PT.1`** | PT | A | M | 4 | ready |
| 11 | **`PT.5`** | PT | A | M | 4 | `PT.2` |
| 12 | **`AT.1`** | AT | A | L | 4 | ready |
| 13 | **`GT.1`** | GT | A | M | 2 | ready |
| 14 | **`IG.13`** | IG | A | M | 2 | ready |
| 15 | **`SC.1`** | SC | A | M | 2 | ready |
