...
### Start here → `IG.8` · Isaac glue integrity

> **Remove the normals fp16 carve-out.** `gbuffer_isaac.py:156-158` allows float16 through a physics plane on the claim that "the renderer delivers the normals AOV as float16 whether we like it or not (measured; ADR 0014 addendum)".

`IG.8` is phase 0, size S, and unblocks 0 other step(s).
...
| 5 | **`RP.2`** | RP | 0 | M | — | ready |
| 6 | **`RP.5`** | RP | 0 | M | — | ready |
| 7 | **`PT.2`** | PT | A | M | 7 | ready |
| 8 | **`PT.1`** | PT | A | M | 4 | ready |
| 9 | **`PT.5`** | PT | A | M | 4 | `PT.2` |
| 10 | **`AT.1`** | AT | A | L | 4 | ready |
| 11 | **`GT.1`** | GT | A | M | 2 | ready |
| 12 | **`IG.13`** | IG | A | M | 2 | ready |
| 13 | **`SC.1`** | SC | A | M | 2 | ready |
| 14 | **`AT.2`** | AT | A | S | 1 | ready |
| 15 | **`SC.3`** | SC | A | M | 1 | ready |
