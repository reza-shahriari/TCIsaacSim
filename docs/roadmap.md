...
### Start here → `SC.2` · Sensor chain and published-reference anchoring

> **Mutation test on the electron path**, and substitution of the measured 3-D ratios. Perturbing `read_noise_e` must move the rendered σ. It cannot today, for a reason that is *wiring*, not noise-space correctness — which is why this sits behind SC.1 rather than beside it.

`SC.2` is phase A, size S, and unblocks 0 other step(s).
...
| 4 | **`IG.2`** | IG | A | M | — | ready |
| 5 | **`SC.4`** | SC | A | M | — | ready |
| 6 | **`WM.1`** | WM | B | M | 10 | ready |
| 7 | **`WM.2`** | WM | B | M | 9 | `WM.1` |
| 8 | **`WM.3`** | WM | B | M | 8 | `WM.2` |
| 9 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 10 | **`WM.5`** | WM | B | S | 1 | `WM.3` |
