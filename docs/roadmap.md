...
|---|---|---|---|---|---|
| RP.1 | Restore README's four broken status cells. `materials`, `detector`, `noise` and `isp` carry CHANGELOG prose where the state emoji belongs, so the table is malformed from row 54 down. Restore the MP.5 limitation paragraph and the 135 deleted CHANGELOG lines. | Parser asserts every package has a status row whose 2nd cell is one of four state tokens and whose 3rd is a tier. Red on 4 of 15 rows. `git diff HEAD` shows zero CHANGELOG deletions after. | — | S | 0 |
