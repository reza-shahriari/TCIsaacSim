...
### Start here → `RP.5` · Repair

> ADR status hygiene. No ADR is ever marked Superseded although the template offers it. ADR 0059 says bolometer smear is "never a second blur" while `optics/stage.py:77` applies one at duty 1.0 (ADR 0077). ADR 0006's use case was removed by 0014; 0048's range clause by 0071.

`RP.5` is phase 0, size M, and unblocks 0 other step(s).
...
|---|---|---|---|---|---|---|
| 1 | **`RP.5`** | RP | 0 | M | — | ready |
| 2 | **`PT.2`** | PT | A | M | 7 | ready |
| 3 | **`PT.1`** | PT | A | M | 4 | ready |
| 4 | **`PT.5`** | PT | A | M | 4 | `PT.2` |
| 5 | **`AT.1`** | AT | A | L | 4 | ready |
| 6 | **`GT.1`** | GT | A | M | 2 | ready |
| 7 | **`IG.13`** | IG | A | M | 2 | ready |
| 8 | **`SC.1`** | SC | A | M | 2 | ready |
| 9 | **`AT.2`** | AT | A | S | 1 | ready |
| 10 | **`SC.3`** | SC | A | M | 1 | ready |
| 11 | **`AT.3`** | AT | A | S | — | `AT.2` |
| 12 | **`AT.5`** | AT | A | S | — | ready |
| 13 | **`IG.6`** | IG | A | S | — | ready |
| 14 | **`SC.2`** | SC | A | S | — | `SC.1` |
| 15 | **`AT.4`** | AT | A | M | — | ready |
