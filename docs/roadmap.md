...
### Start here → `SC.1` · Sensor chain and published-reference anchoring

> **Wire the electron budget for photon FPAs.** `pipeline/core.py:176` calls `anchor_noise` unconditionally for both FPA kinds with no `dark_electrons=`, and `electron_budget`/`dark_electrons_for` have no caller in `src/`. M11.6's row claims the opposite.

`SC.1` is phase A, size M, and unblocks 2 other step(s).

#### Then, in order — 89 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`SC.1`** | SC | A | M | 2 | ready |
| 2 | **`SC.3`** | SC | A | M | 1 | ready |
| 3 | **`AT.5`** | AT | A | S | — | ready |
| 4 | **`IG.6`** | IG | A | S | — | ready |
| 5 | **`SC.2`** | SC | A | S | — | `SC.1` |
| 6 | **`AT.4`** | AT | A | M | — | ready |
...
| 8 | **`IG.2`** | IG | A | M | — | ready |
| 9 | **`SC.4`** | SC | A | M | — | ready |
| 10 | **`WM.1`** | WM | B | M | 10 | ready |
| 11 | **`WM.2`** | WM | B | M | 9 | `WM.1` |
| 12 | **`WM.3`** | WM | B | M | 8 | `WM.2` |
| 13 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 14 | **`WM.5`** | WM | B | S | 1 | `WM.3` |
