...
### Start here → `PT.5` · Point-wise surface temperature

> **Point-wise on a moving prim.** `point_bridge.py:98-103` rejects any patch frame but `world`, while `surface_field.py:78-80` documents a prim path as supported — the docstring promises what the code refuses. Compose world-from-local from the prim's USD transform at the frame's timestamp.

`PT.5` is phase A, size M, and unblocks 4 other step(s).

#### Then, in order — 94 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`PT.5`** | PT | A | M | 4 | ready |
| 2 | **`AT.1`** | AT | A | L | 4 | ready |
| 3 | **`GT.1`** | GT | A | M | 2 | ready |
| 4 | **`IG.13`** | IG | A | M | 2 | ready |
| 5 | **`SC.1`** | SC | A | M | 2 | ready |
| 6 | **`AT.2`** | AT | A | S | 1 | ready |
| 7 | **`SC.3`** | SC | A | M | 1 | ready |
| 8 | **`AT.3`** | AT | A | S | — | `AT.2` |
| 9 | **`AT.5`** | AT | A | S | — | ready |
| 10 | **`IG.6`** | IG | A | S | — | ready |
| 11 | **`SC.2`** | SC | A | S | — | `SC.1` |
| 12 | **`AT.4`** | AT | A | M | — | ready |
| 13 | **`GT.2`** | GT | A | M | — | `SC.1` |
| 14 | **`IG.2`** | IG | A | M | — | ready |
| 15 | **`SC.4`** | SC | A | M | — | ready |
