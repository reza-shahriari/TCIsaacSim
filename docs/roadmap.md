...
### Start here → `PT.4` · Point-wise surface temperature

> **Guard the in-plane axes in `patch_view_factors`.** It checks the normals are parallel, then compares an offset measured in the receiver's axes against extents given in the radiator's — so an in-plane rotation is evaluated as if the rectangle had rotated too.

`PT.4` is phase 0, size S, and unblocks 0 other step(s).

#### Then, in order — 101 open steps
...
| 4 | **`RP.2`** | RP | 0 | M | — | ready |
| 5 | **`RP.5`** | RP | 0 | M | — | ready |
| 6 | **`PT.2`** | PT | A | M | 7 | ready |
| 7 | **`PT.1`** | PT | A | M | 4 | ready |
| 8 | **`PT.5`** | PT | A | M | 4 | `PT.2` |
| 9 | **`AT.1`** | AT | A | L | 4 | ready |
| 10 | **`GT.1`** | GT | A | M | 2 | ready |
| 11 | **`IG.13`** | IG | A | M | 2 | ready |
| 12 | **`SC.1`** | SC | A | M | 2 | ready |
| 13 | **`AT.2`** | AT | A | S | 1 | ready |
| 14 | **`SC.3`** | SC | A | M | 1 | ready |
| 15 | **`AT.3`** | AT | A | S | — | `AT.2` |

…and 86 more — `python scripts/next_step.py --queue 40`.
