...
### Start here → `IG.13` · Isaac glue integrity

> **Float32 planes and a config-hash sidecar from every render driver.** `write_frame` appears only in `render_aerial_demo`, `render_maritime_demo` and `render_car_ignition`; `quad_flight`, `aircraft_pass` and `vessel_departure` are PNG-only — the aerial and maritime flight renders. `render_multiband.

`IG.13` is phase A, size M, and unblocks 2 other step(s).

#### Then, in order — 90 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`IG.13`** | IG | A | M | 2 | ready |
| 2 | **`SC.1`** | SC | A | M | 2 | ready |
| 3 | **`SC.3`** | SC | A | M | 1 | ready |
| 4 | **`AT.5`** | AT | A | S | — | ready |
| 5 | **`IG.6`** | IG | A | S | — | ready |
| 6 | **`SC.2`** | SC | A | S | — | `SC.1` |
| 7 | **`AT.4`** | AT | A | M | — | ready |
...
| 11 | **`WM.1`** | WM | B | M | 10 | ready |
| 12 | **`WM.2`** | WM | B | M | 9 | `WM.1` |
| 13 | **`WM.3`** | WM | B | M | 8 | `WM.2` |
| 14 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 15 | **`WM.5`** | WM | B | S | 1 | `WM.3` |
