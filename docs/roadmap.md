...
### Start here → `SC.3` · Sensor chain and published-reference anchoring

> **Boson datasheet corrections.** FLIR publishes no scalar NETD; it publishes 3-D components with conditions. Fix `ffc_interval_s` 180 → 300 (the factory FFC Period, with its 1.0 °C trigger and 90 s start-up at one-third delta), `thermal_time_constant_ms` 10.0 → 8 ms published, and add the missing ES

`SC.3` is phase A, size M, and unblocks 1 other step(s).

#### Then, in order — 88 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`SC.3`** | SC | A | M | 1 | ready |
| 2 | **`AT.5`** | AT | A | S | — | ready |
| 3 | **`IG.6`** | IG | A | S | — | ready |
| 4 | **`SC.2`** | SC | A | S | — | ready |
| 5 | **`AT.4`** | AT | A | M | — | ready |
...
| 7 | **`IG.2`** | IG | A | M | — | ready |
| 8 | **`SC.4`** | SC | A | M | — | ready |
| 9 | **`WM.1`** | WM | B | M | 10 | ready |
| 10 | **`WM.2`** | WM | B | M | 9 | `WM.1` |
| 11 | **`WM.3`** | WM | B | M | 8 | `WM.2` |
| 12 | **`PT.9`** | PT | A | M | 3 | `WM.3` |
| 13 | **`WM.5`** | WM | B | S | 1 | `WM.3` |
