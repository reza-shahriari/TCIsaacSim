...
### Start here → `AT.1` · Atmosphere, sky and materials

> **Per-pixel slant path.** `pipeline/atmosphere.py:72` passes elevation `0.0` unconditionally and `warp_stages.py:936` mirrors it, so every *resolved* pixel gets surface-density extinction over its whole slant range — while the unresolved point-target path uses `target.elevation_rad`, as does the sky

`AT.1` is phase A, size L, and unblocks 4 other step(s).

#### Then, in order — 93 open steps

| # | step | lane | phase | size | unblocks | waiting on |
|---|---|---|---|---|---|---|
| 1 | **`AT.1`** | AT | A | L | 4 | ready |
| 2 | **`GT.1`** | GT | A | M | 2 | ready |
| 3 | **`IG.13`** | IG | A | M | 2 | ready |
| 4 | **`SC.1`** | SC | A | M | 2 | ready |
| 5 | **`AT.2`** | AT | A | S | 1 | ready |
| 6 | **`SC.3`** | SC | A | M | 1 | ready |
| 7 | **`AT.3`** | AT | A | S | — | `AT.2` |
| 8 | **`AT.5`** | AT | A | S | — | ready |
| 9 | **`IG.6`** | IG | A | S | — | ready |
| 10 | **`SC.2`** | SC | A | S | — | `SC.1` |
| 11 | **`AT.4`** | AT | A | M | — | ready |
| 12 | **`GT.2`** | GT | A | M | — | `SC.1` |
| 13 | **`IG.2`** | IG | A | M | — | ready |
| 14 | **`SC.4`** | SC | A | M | — | ready |
| 15 | **`WM.1`** | WM | B | M | 10 | ready |
