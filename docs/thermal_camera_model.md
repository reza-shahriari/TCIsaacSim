...
Add a small extra kernel (Gaussian/exponential) for bolometer thermal cross-talk between neighboring pixels if you want to match a real part's datasheet MTF. System MTF ≈ product of the above; convert to an equivalent PSF (or approximate with a matching-width Gaussian) and convolve the noise-free point-wise image with it.
